...
const SYNC_STRATEGIES = [
  ...
  { value: 'incremental', label: '增量翻译', description: '仅翻译最近修改过的文件' },
  { value: 'manual', label: '手动触发', description: '仅在手动触发时执行翻译' },
]

const TRIGGER_MODES = [
  { value: 'webhook', label: '自动 (Webhook)', description: 'Push 事件自动触发翻译' },
  { value: 'manual', label: '手动', description: '仅在手动触发时运行翻译' },
]
...
function sanitizeTargetLanguages(baseLanguage: string, languages: string[]) {
  return Array.from(new Set(languages.map((item) => item.trim()).filter((item) => item && item !== baseLanguage)))
}
...
export default function RepositoryConfigClientPage({ initialUser }: RepositoryConfigClientPageProps) {
  ...
  const fetchConfig = useCallback(async () => {
    try {
      ...
      if (data.config) {
        ...
        setSelectedLanguages(sanitizeTargetLanguages(data.config.baseLanguage || 'auto', data.config.targetLanguages || []))
      }
  ...
  const handleSave = async () => {
    try {
      ...
      const sanitizedTargetLanguages = sanitizeTargetLanguages(config.baseLanguage, selectedLanguages)

      if (sanitizedTargetLanguages.length === 0) {
        toast({
          title: '校验失败',
          description: '目标语言不能包含基准语言，请重新选择',
          variant: 'warning',
        })
        return
      }
  ...
  const handleToggleLanguage = (langCode: string) => {
    setSelectedLanguages((prev) => {
      ...
      return sanitizeTargetLanguages(config.baseLanguage, next)
    })
  }

  useEffect(() => {
    setSelectedLanguages((prev) => sanitizeTargetLanguages(config.baseLanguage, prev))
  }, [config.baseLanguage])
          ...
          <div className="p-6 space-y-4">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1 flex items-center gap-1">
                <Info className="w-3 h-3" />
                {engine.engineType === 'openrouter'
                  ? 'OpenRouter 支持多种模型，选择自定义可输入任意 OpenRouter 支持的模型 ID'
                  : 'OpenAI 官方模型'}
              </p>
            </div>
